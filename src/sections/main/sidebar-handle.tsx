...
import { Handle, Toggle } from '../../components/sidebar';
import { useAtom, useAtomValue } from 'jotai';
import { activeBuildNameAtom, buildNamesAtom } from '../../store/builds/builds';
import { MouseEventHandler, useCallback } from 'react';

export function SidebarHandle({ className }: { className?: string }) {
  return (
    <Handle
      className={clsx('items-stretch justify-between p-2 flex flex-col text-stone-500', className)}
    >
      <Builds />
      <Toggle className="p-1">
...
function Builds() {
  ...
  const onBuildIconClick = useCallback<MouseEventHandler<HTMLElement>>(
    (ev) => {
      if (ev.currentTarget.dataset.build) setActiveBuild(ev.currentTarget.dataset.build);
    },
    [setActiveBuild]
  );
  ...
  const onNewBuildClick = useCallback<MouseEventHandler<HTMLElement>>(() => {
    ...
    const sidebarCheckbox = document.querySelector<HTMLInputElement>('input#settings');
    const buildNameInput = document.querySelector<HTMLInputElement>('input#save-build-name');
  ...
  return (
    <div className="flex items-center flex-col gap-2">
      ...
      <div
        ...
        className="p-2 btn btn-square hover:scale-110 btn-sm btn-ghost hover:text-stone-300 rounded-md grid place-content-center"
      >
        <div className="rounded font-bold border-2 border-current w-8 h-8 grid place-content-center">
          +
        </div>
      </div>
    </div>
