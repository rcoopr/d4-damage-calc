...
export function BuildNameInputForm({ name }: { name: string }) {
  ...
  const validate = useCallback(
    (v: string) => v === name || (!buildNames.includes(v) && isValidObjectKey(v)),
    [buildNames, name]
  );
  ...
  const onSubmit = useCallback(
      ...
      if (!editing) {
        if (name === data.name) {
          ...
          setStorage((storage) => {
            ...
            const copiedValue = Object.getOwnPropertyDescriptor(newStorage, name);
    ...
    [editing, name, setStorage, activeBuidName, buildNames, setActiveBuildName]
  );
